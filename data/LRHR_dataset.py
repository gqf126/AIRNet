...
class LRHRDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #     img_HR = util.modcrop(img_HR, 512)
        ...
        if self.paths_LR:
            ...
            if self.opt['phase'] != 'train':
                # img_LR = util.modcrop(img_LR, 512)
                hh, ww, _ = img_HR.shape
                # print('*******',hh,ww)
                r_h = random.randint(0, max(0, hh-512))
                r_w = random.randint(0, max(0, ww-512))
                img_HR = img_HR[r_h : r_h + 512, r_w : r_w + 512, :]
                img_LR = img_LR[r_h : r_h + 512, r_w : r_w + 512, :]
                # print('++++++++++')
