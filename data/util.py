...
def channel_convert(in_c, tar_type, img_list):
    ...
    if in_c == 3 and tar_type == 'gray':  # BGR to gray
        ...
        ycbcr_list = [bgr2yuv_convention(img) for img in img_list]
        return ycbcr_list
    elif in_c == 3 and tar_type == 'ycbcr_matlab':  # BGR to y
        ycbcr_list = [bgr2ycbcr(img, only_y=False) for img in img_list]
        return ycbcr_list
    elif in_c == 3 and tar_type == 'lab':  # BGR to y
        lab_list = [bgr2lab(img, only_y=False) for img in img_list]
        return lab_list       
...
def rgb2yuv_convention(img):
    ...
    # rlt = np.matmul(img, [[24.966, 112.0, -18.214], [128.553, -74.203, -93.786],
    #                           [65.481, -37.797, 112.0]]) / 255.0 + [16, 128, 128]
    rlt = np.matmul(img, np.array([[0.29900, -0.16874, 0.50000],
                                   [0.58700, -0.33126, -0.41869],
                                   [0.11400, 0.50000, -0.08131]])) + [0, 128.0, 128.0]
...
def bgr2yuv_convention(img):
    ...
    # rlt = np.matmul(img, [[24.966, 112.0, -18.214], [128.553, -74.203, -93.786],
    #                           [65.481, -37.797, 112.0]]) / 255.0 + [16, 128, 128]
    rlt = np.matmul(img, np.array([[0.11400, 0.50000, -0.08131],
                                   [0.58700, -0.33126, -0.41869],
                                   [0.29900, -0.16874, 0.50000]])) + [0, 128.0, 128.0]
...
def bgr2lab(img, only_y=True):
    '''bgr version of rgb2lab
    ...
    # convert
    if only_y:
        # rlt = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        rlt = color.rgb2lab(img.astype(np.floating))
        rlt = rlt[:, :, 0] / 100
    else:
        # rlt = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        rlt = color.rgb2lab(img.astype(np.floating))
        # L[0, 1], a[-1, 1], b[-1, 1]
        rlt = rlt / np.array([100., 127., 127.], dtype=np.float32)
        # L[0, 1], a[0, 1], b[0, 1]
        # rlt = (rlt + np.array([0., 128., 128.], dtype=np.float32)) / np.array([100., 255., 255.], dtype=np.float32)
...
def lab2rgb(img):
    ...
    # rlt = color.lab2rgb(img.astype(np.floating))
